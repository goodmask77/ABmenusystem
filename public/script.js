...
// 將本地訂單同步到 Supabase（批次上傳）
async function syncLocalOrdersToSupabase() {
    const localMenus = getSavedMenus();
    
    // 過濾出尚未同步到雲端的訂單（排除自動同步記錄）
    const unsyncedMenus = localMenus.filter(menu => 
        !menu.fromSupabase && 
        !menu.autoGenerated && 
        menu.name && 
        menu.name !== '最新同步'
    ...
    if (unsyncedMenus.length === 0) {
        showSyncStatus('沒有需要同步的本地訂單', 'success');
        return 0;
    }
    ...
    let successCount = 0;
    showSyncStatus(`正在同步 ${unsyncedMenus.length} 筆訂單...`, 'pending');
    ...
    showSyncStatus(`已同步 ${successCount}/${unsyncedMenus.length} 筆訂單到雲端`, 'success');
